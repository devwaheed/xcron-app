...
export default function ActionCard({ action, onToggle, onTrigger, onDelete, onDuplicate, toggling = false, triggering = false }: ActionCardProps) {
  ...
  return (
    <div className={`group relative rounded-2xl border border-slate-200/60 bg-white/70 shadow-sm shadow-slate-200/50 backdrop-blur-xl transition-all hover:border-slate-300/80 hover:shadow-md hover:shadow-slate-200/60 ${isPaused ? "opacity-60" : ""}`}>
      ...
      <div className="flex items-center gap-1 border-t border-slate-100/80 px-4 py-2.5">
        ...
        {/* Duplicate */}
        {onDuplicate && (
          <button onClick={() => onDuplicate(action.id)}
            disabled={busy}
            title="Duplicate job"
            aria-label="Duplicate job"
            className="inline-flex items-center justify-center rounded-lg p-2 text-slate-400 transition-all hover:bg-slate-50 hover:text-slate-600 disabled:opacity-50">
